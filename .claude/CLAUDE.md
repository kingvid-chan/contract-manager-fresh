...
8. 静态资源 URL 必须带 `?v=<当前 0.0.N>` 版本令牌、HTML 文档必须由服务器下发真实 `Cache-Control: no-cache` **HTTP 响应头**（不能用 `<meta>` 标签代替），且资源路径保留 `/projects/contract-manager-fresh/` 前缀；否则 Hermes 浏览器验收会拒。
